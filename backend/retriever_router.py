...
RETRIEVER_CAPABILITIES: dict[str, RetrieverCapability] = {
    ...
    "self_query": RetrieverCapability("self_query", "Self-query retriever", "structured query parser + metadata filter scoring without external LLM filters", "scoped document, section, revision, page, table, safety, identifier filtering"),
    ...
    "agentic": RetrieverCapability("agentic", "Agentic retriever", "optional CrewAI retrieval planner with deterministic fallback and bounded local expansion", "not-enough-context expansion"),
    "graph": RetrieverCapability("graph", "Graph retriever", "local entity-edge graph retriever with seed nodes, multi-hop graph expansion, edge evidence, chunk-distance scoring, and graph-neighbor candidate expansion", "connected engineering systems"),
    "knowledge_graph": RetrieverCapability("knowledge_graph", "Knowledge graph retriever", "local entity-relation graph over chunks, sections, documents, standards, and references", "enterprise knowledge relationships"),
    ...
    "late_interaction": RetrieverCapability("late_interaction", "Late interaction retriever", "token-level max-sim scorer with exact, stem, identifier, numeric, fuzzy, and character n-gram matching", "precise term alignment"),
    "memory": RetrieverCapability("memory", "Memory retriever", "chat-history source memory with follow-up query expansion, previous source IDs, sections, entities, and standards", "conversation continuity"),
    ...
    "sql_database": RetrieverCapability("sql_database", "SQL/database retriever", "safe parameterized SQLite retriever over documents, chunks, metadata, retrieval logs, and chat history", "structured local database facts"),
    "api": RetrieverCapability("api", "API retriever", "guarded allowlisted HTTP connector that calls configured endpoints, normalizes JSON/text responses into evidence rows, and scores API results without inventing live data", "live external or internal system evidence when configured"),
    "multi_hop": RetrieverCapability("multi_hop", "Multi-hop retriever", "staged hop planner with hop query expansion, per-hop scoring, linked chunk expansion, section siblings, and relationship evidence", "staged relationship retrieval"),
    "iterative": RetrieverCapability("iterative", "Iterative retriever", "bounded first-pass analysis, missing-term detection, focused retry queries, candidate merge, and iterative rescoring", "retrieve-analyze-expand loop"),
    "query_decomposition": RetrieverCapability("query_decomposition", "Query decomposition retriever", "typed subquestion parser with subquestion query expansion and per-subquestion coverage scoring", "multi-part questions"),
    "tool_aware": RetrieverCapability("tool_aware", "Tool-aware retriever", "local tool planner selects vector, keyword, table, SQL, graph, memory, iterative, multimodal, citation, and late-interaction tools, then expands queries and rescoring candidates", "dynamic retrieval tool choice"),
    ...
    "ontology": RetrieverCapability("ontology", "Ontology retriever", "local engineering ontology with concepts, synonyms, broader categories, related terms, query expansion, metadata matching, and candidate rescoring", "engineering taxonomy and synonym-aware retrieval"),
    "symbolic": RetrieverCapability("symbolic", "Symbolic retriever", "local rule engine for obligations, prohibitions, conditions, exceptions, permissions, recommendations, definitions, numeric constraints, and safety-critical wording with query expansion and candidate rescoring", "rule-based engineering requirement retrieval"),
    "semantic_graph": RetrieverCapability("semantic_graph", "Semantic graph retriever", "fuses dense/vector relevance with knowledge-graph nodes, ontology concepts, relationship types, section links, and lexical query terms", "hybrid graph plus semantic retrieval"),
...
ROUTES: dict[str, RetrieverRoute] = {
    ...
    "agentic_iterative": RetrieverRoute("agentic_iterative", ("retriever_routing_system", "agentic", "iterative", "multi_hop", "query_decomposition", "hybrid", "api"), 0.32, 0.30, 0.12, 0.18, 0.02, 0.06, 0.04, 1, True, True, True, route_notes="CrewAI-assisted bounded retrieval planning plus multi-hop section/link/entity expansion for complex questions."),
    "graph": RetrieverRoute("graph", ("retriever_routing_system", "graph", "knowledge_graph", "semantic_graph", "entity", "ontology", "symbolic"), 0.28, 0.36, 0.10, 0.16, 0.02, 0.08, 0.06, 1, True, True, graph_mode=True, route_notes="Entity, relationship, section, document, standard, and reference graph retrieval."),
    "memory": RetrieverRoute("memory", ("retriever_routing_system", "memory", "cache", "hybrid", "multi_query"), 0.30, 0.30, 0.10, 0.15, 0.02, 0.13, 0.04, multi_query=True, cache_mode=True, route_notes="Conversation-aware retrieval using recent turns, previous source chunks, sections, entities, and standards."),
    ...
    "tool_aware": RetrieverRoute("tool_aware", ("retriever_routing_system", "tool_aware", "sql_database", "api", "metadata", "hybrid", "symbolic"), 0.26, 0.34, 0.12, 0.14, 0.04, 0.10, 0.06, diversify_sections=True, route_notes="Dynamic local tool planner for structured, table, metadata, vector, graph, memory, citation, precision, and configured API retrieval."),
...
def route_for_question(profile: QuestionProfile, question: str) -> RetrieverRoute:
    ...
    if re.search(r"\b(table|row|column|slope|rating|dimension|value|mm|1:\d+|matrix)\b", normalized):
        return ROUTES["table"]
    if re.search(r"\b(valves?|isolation|actuated|manual valves?|relief valves?|rupture discs?|pressure relief)\b", normalized):
